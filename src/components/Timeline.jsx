...
function statusPhaseColor(status) {
  return STATUS_COLOR[status] ?? '#444444'
}

// ─── Starburst ────────────────────────────────────────────────────────────────

function Starburst({ word, status }) {
  const color = statusPhaseColor(status)
  return (
    <div
      className="tl-starburst"
      style={{
    ...
    >
      <span style={{
        fontFamily: "'Bangers', cursive",
        fontSize: '0.65rem',
        color: status === 'upcoming' ? '#888' : '#0A0A0A',
        letterSpacing: '0.02em',
        textAlign: 'center',
        lineHeight: 1.1,
        padding: '0 4px',
...
function StatusBadge({ status }) {
  const map = {
    ...
    active:    { label: 'LIVE NOW',  color: '#FF2D55' },
    upcoming:  { label: 'UPCOMING',  color: '#444' },
  }
  ...
  return (
    <div style={{
      ...
      {status === 'active' && (
        <span
          className="live-dot"
          style={{ width: 8, height: 8, borderRadius: '50%', background: '#FF2D55', display: 'inline-block' }}
        />
      )}
      <span style={{
        fontFamily: "'Plus Jakarta Sans', sans-serif",
  ...
  return (
    <div
      ...
      style={{
        ...
        flexShrink: 0,
        background: '#111',
        border: `2px solid ${status === 'active' ? '#FF2D55' : '#1E1E1E'}`,
        boxShadow: status === 'active' ? '5px 5px 0 #FF2D55' : '5px 5px 0 #1A1A1A',
        padding: '1.5rem',
        position: 'relative',
        overflow: 'hidden',
...
// ─── Connector (always horizontal, desktop only) ──────────────────────────────
...
function VerticalConnector({ prevStatus }) {
  const color = prevStatus === 'completed' ? '#00D4FF'
              : prevStatus === 'active'    ? '#FF2D55'
              : '#2A2A2A'
  return (
...
// ─── Mobile Timeline (pure vertical, zero GSAP pinning) ───────────────────────
...
function MobileTimeline() {
  ...
  useGSAP(() => {
    const ctx = gsap.context(() => {
      const panels = listRef.current?.querySelectorAll('.tl-panel')
    ...
    <div ref={listRef} style={{ display: 'flex', flexDirection: 'column' }}>
      {timeline.map((phase, i) => (
        <div key={phase.phase}>
          <PhasePanel phase={phase} />
          {i < timeline.length - 1 && (
            <VerticalConnector prevStatus={phase.status} />
          )}
        </div>
      ))}
    </div>
...
function DesktopTimeline() {
  ...
  return (
    <div ref={sectionRef} style={{ position: 'relative' }}>
      {/* Progress bar */}
      <div style={{ background: '#1A1A1A', height: '3px', marginBottom: '2.5rem', position: 'relative' }}>
        <div
          ref={progressRef}
          style={{
            position: 'absolute', left: 0, top: 0, bottom: 0,
...
export default function Timeline() {
  ...
  // Detect mobile via matchMedia — this is evaluated once on mount
  // and is SSR-safe (no window access during render)
  ...
  return (
    <section
      id="timeline"
      ref={sectionRef}
      style={{ background: '#0A0A0A', position: 'relative', overflow: isMobile ? 'visible' : 'hidden' }}
    >
      {/* Halftone overlay */}
      <div style={{
        position: 'absolute', inset: 0,
